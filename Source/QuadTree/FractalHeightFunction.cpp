...
double FractalHeightFunction::get_value(glm::highp_dvec3 point) {
	...
	result = ((1.0 - glm::simplex(temp * restriction)) + _offset) * exponentials.at(0);
	weight = result;
	temp.x *= _lacunarity;
	temp.y *= _lacunarity;
	temp.z *= _lacunarity;
	...
	for (i = 1; i < _octaves; ++i) {
		...
		signal = ((1.0 - glm::simplex(temp * restriction)) + _offset) * exponentials.at(i);
	...
	if (remainder > 0.0) {
		result += remainder * (1.0 - glm::simplex(temp * restriction)) * exponentials.at(i);
	}
